...
func runInstall(cmd *cobra.Command, args []string) error {
	...
	// 0.0.0.0 (all interfaces) is the new default. The bearer token
	// (~/.mcp-tools-web.token) gates access — generated by this command
	// above — so the API is not unauthenticated.
	...
	if bind == "0.0.0.0" || bind == "::" {
		fmt.Fprintf(os.Stdout, "  Auth:   bearer token requerido (Authorization: Bearer <token>)\n")
	}
