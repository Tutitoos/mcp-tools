...
var installCmd = &cobra.Command{
	...
	Long: "Genera el token, escribe el unit file mcp-tools-web.service, lo habilita y lo inicia. " +
...
func runInstall(cmd *cobra.Command, args []string) error {
	...
	if bind != "127.0.0.1" && bind != "::1" && !installAllowPublic {
		return fmt.Errorf("bind=%q expone el panel fuera de loopback; usa --allow-public-bind o elige 127.0.0.1", bind)
	}

	if _, err := writeToken(); err != nil {
		return err
	}
	tp, _ := tokenPath()
	fmt.Fprintf(os.Stdout, "token generado en %s\n", tp)
